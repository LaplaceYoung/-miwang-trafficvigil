...
function SearchBox({ value, onChange, placeholder, label = "搜索", className = "" }) {
  return (
    <div role="search" className={cx("flex min-h-9 items-center gap-2 rounded-lg border border-cyan-300/15 bg-slate-950/35 px-3 text-sm text-slate-300 focus-within:border-cyan-300/55 focus-within:bg-slate-950/55", className)}>
      <Search size={15} className="text-slate-500" />
      <input aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="tv-search-input w-full border-0 bg-transparent p-0 text-sm text-slate-100 outline-none placeholder:text-slate-500" />
    </div>
  );
}

function EmptyTableState({ message = "当前筛选条件下暂无数据" }) {
  return <div className="rounded-xl border border-dashed border-cyan-300/18 bg-slate-950/25 px-4 py-8 text-center text-sm text-slate-400">{message}</div>;
}
...
function CapturePage({ go }) {
  ...
  const toggleProtocol = (protocol) => {
    setSelectedProtocols((items) => {
      const next = items.includes(protocol) ? items.filter((item) => item !== protocol) : [...items, protocol];
      toast(`${protocol} 过滤${next.includes(protocol) ? "已启用" : "已停用"}`);
      return next;
    });
  };
...
function SelectLine({ text, options = [], onChange }) {
  const [open, setOpen] = useState(false);
  const choices = options.length ? options : [text];
  return (
    <div className="relative">
      <button type="button" onClick={() => setOpen((v) => !v)} aria-expanded={open} className={cx("tv-button flex h-9 w-full items-center justify-between rounded-lg border border-cyan-300/15 bg-slate-950/35 px-3 text-left text-xs text-slate-300", open && "border-cyan-300/45 text-cyan-200")}>{text}<ChevronDown className={cx("transition-transform", open && "rotate-180")} size={15}/></button>
      {open && <div className="absolute left-0 right-0 top-10 z-30 rounded-lg border border-cyan-300/20 bg-slate-950/95 p-1 shadow-xl">{choices.map((item) => <button type="button" key={item} onClick={() => { onChange?.(item); setOpen(false); toast(`已选择 ${item}`); }} className={cx("tv-button block w-full rounded-md px-3 py-2 text-left text-xs hover:bg-cyan-400/10", item === text ? "text-cyan-200" : "text-slate-300")}>{item}</button>)}</div>}
    </div>
  );
}
...
function VpnPage({ go }) {
  ...
  const [vpnFilter, setVpnFilter] = useState("全部");
  const filteredFlowRows = flowRows.filter((row) => rowMatches(row, flowQuery) && (vpnFilter === "全部" || (vpnFilter === "高置信度" ? Number(String(row[7]).replace("%", "")) >= 93 : row[6] === vpnFilter)));
  const startAnalysis = () => {
    setRunning(true);
    setTimeout(() => setRunning(false), 1600);
  };
  const exportVpnCsv = () => {
    downloadText("vpn_classification.csv", toCsv(["flow_id", "source", "destination", "protocol", "packet_count", "vpn_probability", "prediction", "confidence"], filteredFlowRows), "text/csv");
  };
  return (
    <PageShell title="VPN 流量分析" actions={null}>
      <div className="grid grid-cols-[1fr_230px] gap-4">
        <div>
          ...
          <Card className="mt-3"><div className="mb-3 flex items-center justify-between gap-3"><PanelTitle title={`流量分类结果 · ${refreshedAt}`} /><div className="flex flex-wrap gap-2"><SearchBox value={flowQuery} onChange={setFlowQuery} placeholder="搜索 Flow ID / IP / 协议..." className="w-56" /><SmallSelect label={vpnFilter} options={["全部","VPN","非 VPN","高置信度"]} onChange={setVpnFilter} /><Button variant="ghost" icon={RefreshCw} onClick={()=>setRefreshedAt(new Date().toLocaleTimeString("zh-CN", { hour12: false }))}>刷新表格</Button></div></div><DataTable columns={["Flow ID","Source","Destination","Protocol","Packet Count","VPN Probability","Prediction","Confidence"]} rows={filteredFlowRows} renderCell={(c,j)=> j===6 ? <span className={cx("rounded-md border px-2 py-1 text-xs", c==="VPN" ? "border-cyan-400/30 bg-cyan-400/10 text-cyan-300" : "border-purple-400/30 bg-purple-400/10 text-purple-300")}>{c}</span> : c}/><Pagination total={`共 ${filteredFlowRows.length} / 18,642 条`} /></Card>
        </div>
...
function SimPage({ go }) {
  ...
  const [selectedApp, setSelectedApp] = useState("全部应用");
  const [appSort, setAppSort] = useState("按流量占比");
  ...
  const appItems = [{label:"Telegram",value:24.7,color:appMeta.Telegram.color, files: 2},{label:"WhatsApp",value:18.3,color:appMeta.WhatsApp.color, files: 2},{label:"Signal",value:12.6,color:appMeta.Signal.color, files: 1},{label:"WeChat",value:7.8,color:appMeta.WeChat.color, files: 1},{label:"QQ",value:4.9,color:appMeta.QQ.color, files: 1},{label:"Unknown",value:31.7,color:appMeta.Unknown.color, files: 2}];
  const sortedAppItems = [...appItems].sort((a, b) => appSort === "按文件数量" ? b.files - a.files : b.value - a.value);
  const filteredSimRows = simRows.filter((row) => rowMatches(row, simQuery) && (selectedApp === "全部应用" || row[8] === selectedApp));
  const activeAppName = selectedApp === "全部应用" ? "Telegram" : selectedApp;
  const saveSelectedApp = () => setSavedApps((prev) => prev.includes(activeAppName) ? prev : [...prev, activeAppName]);
  const exportSimCsv = () => {
    downloadText("sim_application_probability.csv", toCsv(["flow_id", "top1", "top1_probability", "top2", "top2_probability", "telegram", "whatsapp", "signal", "prediction"], filteredSimRows), "text/csv");
  };
  return (
    <PageShell title="SIM 流量分析系统" subtitle="对过滤后的 VPN 流量进行 IM 应用识别与置信度评估">
      ...
      <div className="mt-4 grid grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] gap-4"><Card><PanelTitle title="SIM / 非 SIM 分类" /><div className="grid grid-cols-[minmax(0,0.9fr)_minmax(0,1fr)] items-center"><Donut center={<><div>总流量</div><b className="text-xl">712.31 GB</b></>} data={[{value:43.4,color:palette.blue},{value:56.6,color:"#cbd5e1"}]} onSegmentClick={(segment, index)=>toast(index === 0 ? "已筛选 SIM 流量" : "已查看非 SIM 流量")} /><Legend items={[["SIM 流量","307.89 GB","43.4%",palette.blue],["非 SIM 流量","404.42 GB","56.6%","#cbd5e1"]]} /></div><div className="mt-3 rounded-lg border border-cyan-300/10 p-3 text-center text-cyan-300">SIM 流量占比：43.4% (307.89 GB)</div></Card><Card><PanelTitle title="IM 应用识别分布" right={<SmallSelect label={appSort} options={["按流量占比","按置信度","按文件数量"]} onChange={setAppSort} />} /><BarChart showAppIcons items={sortedAppItems} onBarClick={(item)=>{ setSelectedApp(item.label); toast(`已筛选 ${item.label} 流量`); }} /></Card></div>
      <div className="mt-4 grid grid-cols-[minmax(0,1.3fr)_minmax(0,0.6fr)] gap-4"><Card><PanelTitle title="应用识别概率" right={<div className="flex flex-wrap gap-2"><SearchBox value={simQuery} onChange={setSimQuery} placeholder="搜索 Flow ID / 应用..." className="w-52" /><SmallSelect label={selectedApp} options={["全部应用","Telegram","WhatsApp","Signal","WeChat","QQ","Unknown"]} onChange={setSelectedApp} /></div>} /><DataTable columns={["Flow ID","Top1","Top1 Probability","Top2","Top2 Probability","Telegram","WhatsApp","Signal","Prediction"]} rows={filteredSimRows} renderCell={(c,j)=> j>=5&&j<=7 ? <span className="block rounded bg-blue-500/20 px-2 py-1 text-center text-blue-100">{c}</span> : j===8 ? <span className="font-bold text-cyan-300">{c}</span> : c}/><Pagination total={`共 ${filteredSimRows.length} / 25,684 条`} /></Card><Card><PanelTitle title="分类导出文件管理" /><DataTable columns={["应用类型","流量大小","文件数量","操作"]} rows={exports.map(e=>[e[0],e[1],e[2],"下载"])} renderCell={(c,j,r)=> j===0 ? <span className="flex items-center gap-2"><AppIcon app={c} size={20} />{c}</span> : j===3 ? <button type="button" aria-label={`下载 ${r[0]} 流量`} onClick={()=>downloadPcap(`${r[0]}.pcap`, `sim-${r[0]}`, Number(r[2]) + 4)} className="text-cyan-300"><Download size={16}/></button> : c}/><div className="mt-4 flex justify-between font-bold"><span>合计</span><span>390.36 GB</span><span>9</span></div></Card></div>
      <Card className="mt-4"><div className="grid grid-cols-3 gap-6"><Button variant="ghost" icon={FileText} onClick={saveSelectedApp}>{savedApps.includes(activeAppName) ? `${activeAppName} 已保存` : `保存 ${activeAppName} 流量`}</Button><Button icon={ArrowRight} onClick={()=>go("behavior")}>进入行为嗅探</Button><Button variant="ghost" icon={Download} onClick={()=>{ exportSimCsv(); downloadPcap("filtered_sim_traffic.pcap", "sim-filtered", 14); }}>导出识别结果</Button></div></Card>
    </PageShell>
  );
}
...
function GroupPage({ go }) {
  ...
  const [riskFilter, setRiskFilter] = useState("风险等级：全部");
  const filteredGroupRows = groupRows.filter((row) => rowMatches(row, groupQuery) && (riskFilter === "风险等级：全部" || row[4] === riskFilter));
  ...
  return (
    <PageShell title="公共群组匹配" subtitle="基于多源特征与关系图谱的群组关联分析与溯源追踪" actions={<><Button variant="ghost" icon={Copy} onClick={copyTaskId}>任务 ID：T20250519001</Button><Button icon={FileText} onClick={()=>go("reports")}>导出报告</Button></>}>
      ...
      <Card className="mt-4"><div className="mb-3 flex items-center justify-between gap-3"><PanelTitle title={`群组匹配结果 · ${refreshedAt}`} /><div className="flex flex-wrap gap-3"><SmallSelect label={riskFilter} options={["风险等级：全部","严重","高危","中危","低危"]} onChange={setRiskFilter}/><SearchBox value={groupQuery} onChange={setGroupQuery} placeholder="搜索 Group ID / Name" className="w-52" /><Button variant="ghost" icon={RefreshCw} onClick={()=>setRefreshedAt(new Date().toLocaleTimeString("zh-CN", { hour12: false }))}>刷新表格</Button></div></div><DataTable columns={["Group ID","Group Name","Platform","匹配概率","风险等级","证据数量","Last Active Time"]} rows={filteredGroupRows} renderCell={(c,j,r)=> j===1 ? <button onClick={()=>setSelected(c)} className="text-left text-cyan-200">{c}</button> : j===3 ? <div className="flex items-center gap-2"><span>{c}</span><div className="h-2 w-24 rounded bg-slate-800"><div className="h-full rounded bg-blue-400" style={{width:c}} /></div></div> : j===4 ? <RiskTag level={c}/> : c}/><Pagination total={`共 ${filteredGroupRows.length} / 128 条`} /></Card>
    </PageShell>
  );
}
...
function ReportsPage({ selectedReport, setSelectedReport }) {
  ...
  const [reportType, setReportType] = useState("全部任务类型");
  const [dateRange, setDateRange] = useState("近 30 日");
  const reportTypeFor = (report) => report[1].includes("VPN") || report[1].includes("异常访问") ? "完整检测" : report[1].includes("识别") || report[1].includes("WiFi") ? "快速检测" : "报告生成";
  const reportInRange = (_report, index) => dateRange === "今日" ? index < 2 : dateRange === "近 7 日" ? index < 4 : true;
